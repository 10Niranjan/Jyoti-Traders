...
void main() {
  ...
  setUp(() async {
    tempDir = Directory.systemTemp.createTempSync(
      'jyoti_traders_order_flow_test_',
    );
  ...
  test('browse -> cart -> checkout -> success', () async {
    ...
    final cartRepo = CartRepositoryImpl(
      CartLocalDatasource(cartBox: cartBox, uid: 'retailer_1'),
    );
    ...
    expect(
      cart.subtotal >= Money(2500),
      isTrue,
    ); // clears the minimum-order gate
  ...
  test(
    'below the ₹2,500 minimum: checkout rejects the order and the cart is untouched',
    () async {
      final cartRepo = CartRepositoryImpl(
        CartLocalDatasource(cartBox: cartBox, uid: 'retailer_1'),
      );
      ...
      expect(
        history.map((o) => o.id),
        isNot(contains('order_should_not_exist')),
      );

      final untouchedCart = await cartRepo.watchCart().first;
      expect(
        untouchedCart.isEmpty,
        isFalse,
      ); // cart is not cleared on rejection
    },
  );
}
