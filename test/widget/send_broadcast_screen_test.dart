...
void main() {
  ...
  testWidgets('shows the empty history state when nothing has been sent', (
    tester,
  ) async {
  ...
  testWidgets(
    'sending a broadcast clears the form, shows a confirmation and lists it in history',
    (tester) async {
  ...
  testWidgets('the form does not submit when title or message is blank', (
    tester,
  ) async {
    final repo = FakeBroadcastRepository();
    await tester.pumpWidget(_wrap(repo));
