...
class FakeCategoryRepository implements CategoryRepository {
  ...
  @override
  Future<void> deleteCategory(String categoryId) async => deletedIds.add(categoryId);
}
...
CategoryEntity _category({
  ...
  bool isActive = true,
    ...
    CategoryEntity(id: id, name: name, iconUrl: '', displayOrder: displayOrder, isActive: isActive);
...
void main() {
  ...
  testWidgets('shows inactive categories too — the admin must be able to re-activate them', (tester) async {
    await tester.pumpWidget(_wrap(FakeCategoryRepository([
  ...
  testWidgets('delete asks for confirmation first and only deletes when confirmed', (tester) async {
    final repo = FakeCategoryRepository([_category(id: 'c1', name: 'Edible Oils')]);
  ...
  testWidgets('dragging a category by its handle persists the new order', (tester) async {
    ...
    await tester.timedDrag(handle, const Offset(0, 200), const Duration(milliseconds: 300));
    await tester.pumpAndSettle();
