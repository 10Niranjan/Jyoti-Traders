...
void main() {
  testWidgets('shows empty state when there are no pending retailers', (
    tester,
  ) async {
  ...
  testWidgets(
    'renders shop name, owner, phone and address for each pending retailer',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            userRepositoryProvider.overrideWithValue(
              FakeUserRepository([_pendingUser('r1')]),
            ),
          ],
          child: const MaterialApp(
            localizationsDelegates: AppLocalizations.localizationsDelegates,
  ...
  testWidgets('shows a fallback when a retailer has no address yet', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          userRepositoryProvider.overrideWithValue(
            FakeUserRepository([userWithoutAddress]),
          ),
        ],
        child: const MaterialApp(
          localizationsDelegates: AppLocalizations.localizationsDelegates,
  ...
  testWidgets(
    'tapping Approve calls the repository, shows a spinner mid-flight, then a success snackbar',
    (tester) async {
      ...
      await tester
          .pump(); // start the async call, don't settle — the gate is still closed
      ...
      expect(
        find.text('Approve'),
        findsNothing,
      ); // buttons swapped for the spinner while busy
  ...
  testWidgets(
    'tapping Reject calls the repository and shows a confirmation snackbar',
    (tester) async {
      final repo = FakeUserRepository([_pendingUser('r1')]);
