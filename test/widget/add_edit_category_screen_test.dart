...
class FakeCategoryRepository implements CategoryRepository {
  ...
  @override
  Future<void> createCategory(CategoryEntity category) async => created.add(category);

  @override
  Future<void> updateCategory(CategoryEntity category) async => updated.add(category);

  @override
  Future<void> deleteCategory(String categoryId) async {}
}

Widget _wrap(FakeCategoryRepository repo, {String? categoryId}) => ProviderScope(
      overrides: [categoryRepositoryProvider.overrideWithValue(repo)],
      child: MaterialApp(home: AddEditCategoryScreen(categoryId: categoryId)),
    );
...
void main() {
  ...
  testWidgets('create mode shows an empty "Add Category" form', (tester) async {
    ...
    expect(find.text('Add Category'), findsNWidgets(2)); // app bar + submit button
  });
  ...
  testWidgets('creates a category placed after the existing ones', (tester) async {
    final repo = FakeCategoryRepository([
      CategoryEntity(id: 'c1', name: 'Existing', iconUrl: '', displayOrder: 0, isActive: true),
    ]);
    await tester.pumpWidget(_wrap(repo));
    await tester.pumpAndSettle();
  ...
  testWidgets('edit mode pre-fills the form from the existing category and updates it', (tester) async {
    final existing = CategoryEntity(id: 'c1', name: 'Edible Oils', iconUrl: '', displayOrder: 2, isActive: true);
    ...
    await tester.enterText(find.widgetWithText(TextFormField, 'Edible Oils'), 'Edible Oils (New)');
    await tester.tap(find.widgetWithText(ElevatedButton, 'Save Changes'));
    await tester.pumpAndSettle();

    expect(repo.updated, hasLength(1));
    expect(repo.updated.single.id, 'c1'); // keeps its id rather than creating a duplicate
    expect(repo.updated.single.name, 'Edible Oils (New)');
    expect(repo.updated.single.displayOrder, 2); // unchanged by the edit form — reorder is drag-only
    expect(repo.created, isEmpty);
  });
  ...
  testWidgets('turning off Active persists isActive false', (tester) async {
    ...
    await tester.enterText(find.widgetWithText(TextFormField, 'Category name'), 'Snacks');
