...
Widget _wrap(FakeUserRepository userRepo, FakeOrderRepository orderRepo) =>
    ProviderScope(
      overrides: [
...
void main() {
  testWidgets('shows an empty state when there are no approved retailers', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(FakeUserRepository([]), FakeOrderRepository([])),
    );
  ...
  testWidgets('shows every approved retailer even with zero orders', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(
  ...
  testWidgets(
    'computes total spend and order count per retailer, sorted by spend',
    (tester) async {
      await tester.pumpWidget(
        _wrap(
          FakeUserRepository([
            _user('u1', 'Small Spender'),
            _user('u2', 'Big Spender'),
          ]),
          FakeOrderRepository([
            _order(
              id: 'o1',
              userId: 'u1',
              shopName: 'Small Spender',
              subtotal: 1000,
            ),
            _order(
              id: 'o2',
              userId: 'u2',
              shopName: 'Big Spender',
              subtotal: 5000,
            ),
            _order(
              id: 'o3',
