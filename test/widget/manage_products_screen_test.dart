...
class FakeProductRepository implements ProductRepository {
  ...
  @override
  Future<void> deleteProduct(String productId) async => deletedIds.add(productId);
}
...
void main() {
  testWidgets('shows an empty state when the catalog has no products', (tester) async {
  ...
  testWidgets('lists products with price, unit and stock', (tester) async {
    await tester.pumpWidget(_wrap(FakeProductRepository([_product(id: 'p1', name: 'Basmati Rice')])));
    await tester.pumpAndSettle();
  ...
  testWidgets('shows inactive products too — the admin must be able to re-activate them', (tester) async {
    await tester.pumpWidget(_wrap(FakeProductRepository([
    ...
    expect(find.text('Inactive'), findsOneWidget); // badge only on the inactive one
  });

  testWidgets('flags low stock with a banner and a per-row warning', (tester) async {
    await tester.pumpWidget(_wrap(FakeProductRepository([
    ...
    expect(find.byIcon(Icons.warning_amber_rounded), findsNWidgets(2)); // banner + the low row
  });

  testWidgets('does not show the low-stock banner when everything is well stocked', (tester) async {
    await tester.pumpWidget(_wrap(FakeProductRepository([_product(id: 'p1', name: 'Plenty', stock: 40)])));
    await tester.pumpAndSettle();

    expect(find.byIcon(Icons.warning_amber_rounded), findsNothing);
  });

  testWidgets('delete asks for confirmation first and only deletes when confirmed', (tester) async {
    final repo = FakeProductRepository([_product(id: 'p1', name: 'Basmati Rice')]);
