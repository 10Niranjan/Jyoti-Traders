...
class ManageProductsScreen extends ConsumerWidget {
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, ProductEntity product) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text('Delete product?', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 17)),
        content: Text(
    ...
    final success = await ref.read(adminProductControllerProvider.notifier).delete(product.id);
    if (!context.mounted) return;

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(success ? '${product.name} deleted.' : 'Delete failed: ${ref.read(adminProductControllerProvider).error}'),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Manage Products', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
      ),
              ...
              return ListView(
                ...
                children: const [
                  EmptyStateWidget(
                    ...
                    message: 'Tap "Add Product" to create your first catalog item.',
                  ),
                ],
              );
            }

            final lowStockCount = list.where((p) => p.stock <= AppConstants.kLowStockThreshold).length;
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return AdminProductTile(
                  product: product,
                  onEdit: () => context.push(RouteNames.adminEditProductPath(product.id)),
        ...
        children: [
          ...
          Expanded(
            child: Text(
              ...
              style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w600, color: AppColors.error),
            ),
          ),
        ],
      ),
