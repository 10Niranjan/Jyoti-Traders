...
| Phase 5 | Delivery, Payments & Notifications | 🔄 In Progress | 7/10 |
...
> **Status**: IN PROGRESS 🔄 (push notifications + delivery charge complete 2026-07-23)  
...
### Next Immediate Task:
> ✅ Push notifications and real per-km delivery charge calculation are both done — 7/10. `AppConstants.kStubDeliveryCharge` is now only a fallback for addresses without captured GPS coordinates, not the default path. Next: **UPI payment flow** — a UPI QR/ID payment screen at checkout (owner's real UPI details needed from the client — see agents.md's client-spec section for the placeholder), an "I have paid" confirmation button with optional payment-screenshot upload (`ImageUploadService` from Phase 4.3 is reusable here), and an admin action to mark an order paid after UPI confirmation (`OrderEntity.paymentStatus` already exists and is unused by any UI yet — this is what finally reads/writes it). COD needs no further work — it's been fully functional since Phase 3.
