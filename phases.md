...
### Cart nudge, admin banners, PDF invoice & success animation (2026-09-19)
- [x] Cart nudge toward the ₹2,500 minimum — progress bar + suggested products that would close the gap
- [x] Admin-managed Home banners (festival offers / announcements) without an app update
- [x] PDF invoice for any non-cancelled order, shareable to WhatsApp — admin and retailer
- [x] Animated success check on the order-success screen

> **Scope notes**: (1) **Cart nudge** — pure `suggestGapClosers()` (`features/cart/controllers/cart_nudge_controller.dart`): in stock, not already in the cart, and enough stock to actually close the gap; the retailer's regulars first, then whichever product's first add (1 unit, or a round kilo of a weighed product) lands closest to the gap. The rail is the last row of the cart list so it scrolls with the cart instead of squeezing it. Reuses Home's compact "Buy Again" tile, moved from a private class into `shared/widgets/compact_product_tile.dart` so both surfaces share one add/stepper implementation — and fixed a latent bug while moving it: a weighed product's stepper there counted grams one at a time with no unit label. (2) **Banners** — stored as one list in the existing `config/banners` document, so **no Firestore rule change and no new collection**: the `config/{doc}` rule (signed-in read, admin write) already covers it. Same dual-mode datasource pattern as delivery config. Carousel shows the admin's live banners first, then the fixed informational slides. `ponytail:` expiry is evaluated when the provider recomputes, not on a timer. Not built: tapping a banner to open a category or product. (3) **Invoice** — approved new package `pdf` plus four bundled Noto font files (~1.7 MB; the PDF built-in fonts lack ₹ and Devanagari — verified visually with a Marathi shop name). A ~20 KB PDF because fonts are subset. **Plain invoice, no GST split** (user's decision): no per-product GST rate/HSN exists in the data. Seller address/GSTIN are empty constants that print only once set. Shared via `share_plus` `XFile.fromData`, same as the CSV export. `ponytail:` built on the UI isolate. (4) **Success animation** — painted with `CustomPainter` rather than Lottie: no animation file was ever sourced, and a one-shot controller settles under tests. Honours the OS "remove animations" setting. `lottie` remains installed and unused. (5) **Not verified on a physical device** — no device was reachable this session; covered by widget/unit tests, a rendered frame check of the animation and a rendered check of the invoice PDF. The share sheet itself can't be exercised in a test. (6) Tests: 383 → 418. `flutter analyze` zero issues.

### Home search, Settings overhaul, retailer UI polish & admin attention strip (2026-09-19)
- [x] Home search bar works in place; Search-tab icon bug fixed
- [x] Notification switches (Order updates / Promotions / Low-stock) actually gate their notifications
- [x] One shared Settings list for retailer + admin (tappable rows → sheets); WhatsApp support, About/version, Text size, Clear image cache
- [x] Retailer: haptics, cart swipe-to-delete + Undo, pinch-zoom product photo, live order strip on Home, category rail, fly-to-cart animation
- [x] Admin: "Needs attention" strip + week-over-week trend on the Today cards

> **Scope notes**: (1) **Search** — root cause of the icon bug was the theme's `InputDecorationTheme` (`filled` + `enabledBorder`) overriding the pill's `InputBorder.none`, painting a second box inside the pill. New shared `SearchField`/`SearchResultsView` (`features/products/widgets/`) used by Home and the Search tab; back on Home leaves search instead of "Exit app?". (2) **Notification switches were decorative** — saved to the user record, read by nothing. New `notificationPreferencesProvider`; stock alerts, broadcast ingestion and pushed order messages now check it. A broadcast that arrives while Promotions is off is dropped for good rather than replayed later. (3) **Settings** — retailer and admin had two near-copies; now one `SettingsList` (`features/settings/`) with role-specific `extraRows`. Admin gained Language. Retailer `profile_screen.dart` 1,403 → 1,108 lines, admin 398 → 213 (still over rules.md's 300-line screen limit for the retailer one). Two new packages **approved by the user**: `package_info_plus` (About → version) and `flutter_cache_manager` (already a transitive dependency; now direct, for the on-disk cache clear) — `rules.md`/`ARCHITECTURE.md` updated. (4) **Text size** — Auto/Small/Medium/Large (`TextSizeController`); Auto keeps following the phone's font setting clamped to 1.3x. (5) **Real bug found while doing the category rail**: `ProductCard` grids used a fixed `childAspectRatio: 0.62`, tuned at 1.0x text — measured, an out-of-stock card (muted pill + a separate note) needed ~270 dp vs ~255 available (overflow, silently clipped in release), and any larger text size overflowed every card. Fixed at the root: one `productCardHeight()`/`productGridDelegate()` (image + a text block that grows with text scale) now sizes the wishlist, Home Top Products, category and "You may also like" layouts; out-of-stock is now a badge over a dimmed photo (height-neutral). 36 fit-checks (3 widths × 4 text scales × 3 states) guard it. (6) **Category rail** shows only with 2+ categories, switches in place (Back still leaves the screen); at ~130 dp per card `ProductCard` stacks price above its control instead of truncating the price. (7) **Fly-to-cart** aims at the newest mounted cart bar (registry in `fly_to_cart.dart`); only fires when the quantity went *up*. (8) **Attention strip** counts: pending approvals, UPI payments claimed-but-unconfirmed, orders `pending` > 30 min (`kUnconfirmedOrderMinutes`), active products at/below the low-stock threshold; hidden until all sources load so it never claims "all clear" early. Trend compares today *so far* with the same time of day one week ago (a half-finished day isn't shown as a collapse). (9) Deliberately **not** done, per the request: privacy-policy link, delete-account, language-on-first-run. (10) Tests: 88 new; `flutter analyze` zero issues, `flutter test` 506/506. **Not verified on a physical device** — none reachable this session.

### Profile overhaul, admin business details, privacy & account deletion (2026-09-20)
- [x] Retailer Profile tab rebuilt: completeness meter, quick actions, "Your business" insights, each section in its own sheet with per-section Save and a "Discard changes?" guard
- [x] Address polish: default star, Home/Shop/Warehouse label chips, two-letter initials avatar
- [x] Admin **Business details** (legal name, address, GSTIN) → printed as the invoice seller block
- [x] In-app **Delete account** (retailer) and a **Privacy policy** row (appears once a URL is set)

> **Scope notes**: (1) **Why the restructure**: the Profile tab was one long form with a single Save; nothing warned about unsaved edits, and `profile_screen.dart` was 1,108 lines. It is now 267 lines (under the rules.md 300-line limit); header, cards and sections are separate widgets in `features/profile/widgets/`. Edit sheets run `enableDrag: false` because a drag-dismiss calls `Navigator.pop`, which would bypass the `PopScope` guard and silently drop typed edits; errors render inside the sheet (a snackbar would sit behind the modal). (2) **Completeness** counts photo, GPS-tagged address (a typed address without coordinates leaves delivery charge on the flat placeholder, so it doesn't count), GST, payout (UPI alone counts). (3) **Insights** compare this month so far with the *same days* of last month (clamped for shorter months) so the 3rd never reads as a collapse; cancelled orders never count; `trendPercent` moved to `core/utils/trend.dart` and the trend arrow to a shared `TrendLabel`, used by admin and retailer cards. (4) **Business profile** is a single `config/business` document (dual-mode like `config/delivery`); the existing Firestore rule `config/{doc}` already grants signed-in read / admin write, so retailers' phones can build invoices with the owner's details. Logo was in the original idea list and is **not** done (needs an image-embed step in the PDF). (5) **Delete account** is retailer-only — the owner's account is deliberately not self-deletable. Firebase mode re-authenticates with the password, deletes the `users/{uid}` doc, then the login (in that order); orders stay on record for accounting, which the dialog says. Simulation mode removes the simulated user and signs out (passwords aren't stored there). `firestore.rules` now lets a *customer* delete their own user doc — **needs deploying**. `ProfileController.deleteAccount` guards every state write with `mounted`: success signs the user out, tearing down the screen and this autoDispose controller before the await returns (the same failure class as the approval-queue bug in 4.2). (6) `AuthRepository.deleteAccount` is a normal abstract method; a default body doesn't help because `implements` fakes must override every member, so the nine test fakes got a one-line stub. (7) **Privacy policy URL is empty on purpose** (`AppConstants.kPrivacyPolicyUrl`) — no policy exists yet and a link to nothing is worse than none; the row appears automatically once it's set, and the same URL is needed in the Play Console. (8) Tests: 46 new/updated; `flutter analyze` zero issues, `flutter test` 551/551. **Not verified on a physical device.**
